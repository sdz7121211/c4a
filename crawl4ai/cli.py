...
def setup_llm_config() -> tuple[str, str]:
    ...
    if not provider:
        ...
        click.echo("Provider format: 'company/model' (e.g., 'openai/gpt-4o', 'anthropic/claude-3-sonnet', 'hosted_vllm/qwen-max')")
...
def setup_llm_config_extend() -> tuple[str, str, str]:
    provider, token = setup_llm_config()

    config = get_global_config()
    base_url = config.get("DEFAULT_LLM_BASE_URL")

    if not base_url:
        # 'https://dashscope.aliyuncs.com/compatible-mode/v1'
        if provider in ['hosted_vllm/qwen-max']:
            base_url = click.prompt("Enter BASE_URL for " + provider)

    if not config.get("DEFAULT_LLM_PROVIDER") or not config.get("DEFAULT_LLM_PROVIDER_TOKEN") or not config.get("DEFAULT_LLM_BASE_URL"):
        config["DEFAULT_LLM_PROVIDER"] = provider
        config["DEFAULT_LLM_PROVIDER_TOKEN"] = token
        config["DEFAULT_LLM_BASE_URL"] = base_url
        save_global_config(config)
        click.echo("\nConfiguration saved to ~/.crawl4ai/global.yml")

    return provider, token, base_url

async def stream_llm_response(url: str, markdown: str, query: str, provider: str, token: str, base_url: str):
    response = completion(
        model=provider,
        api_key=token,
        base_url=base_url,
...
def crawl_cmd(url: str, browser_config: str, crawler_config: str, filter_config: str, 
    ...
    try:
        ...
        if json_extract is not None:
            ...
            provider, token, base_url = setup_llm_config_extend()
...
Always return valid, properly formatted JSON."""
            ...
            crawler_cfg.extraction_strategy = LLMExtractionStrategy(
                llm_config=LLMConfig(provider=provider, api_token=token, base_url=base_url),
